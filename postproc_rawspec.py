...
def run(argstr, inputs, envvar):
    ...
    if envvar is not None:
        ...
        if "--numa" in envvar:
            ...
            try:
                ...
                membind = int(
                    envvar[numa_argindex + 2]
                )  # safe to assume that this is node number
            except:
            ...
            if cpubind[0] in ["-", "+"]:
                ...
                cpu_info = (
                    subprocess.run("lscpu", capture_output=True).stdout.decode().strip()
                )
                try:
    ...
    if "-d" in rawargs:
        rawspec_outputstem = os.path.join(
            rawargs[rawargs.index("-d") + 1], os.path.basename(inputpath)
        )
...
if __name__ == "__main__":
    run(
        "-f 262144 -t 2 -I 1.0 -d /mnt/buf0/rawspec/bogus",
        ["bogus"],
        "CUDA_VISIBLE_DEVICES:$inst$ --numa -4 0",
    )
    run(
        "-f 262144 -t 2 -I 1.0 -d /mnt/buf1/rawspec/bogus",
        ["bogus"],
        "CUDA_VISIBLE_DEVICES:$inst$ --numa -4 1",
    )
    run(
        "-f 262144 -t 2 -I 1.0 -d /mnt/buf0/rawspec/bogus",
        ["bogus"],
