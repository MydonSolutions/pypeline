...
def run(argstr, inputs, envvar):
    if len(inputs) < 2:
        print(
            "Plotter requires a two input paths, the turbo seti output and rawspec's output."
        )
        return []
    ...
    if len(dat_filepath) > 2:
        print(
            "Plotter requires a two input paths, the turbo seti output and rawspec's output. Ignoring {}".format(
                dat_filepath[1:]
            )
        )
    ...
    parser = argparse.ArgumentParser(
        description="Filter turboseti dat and plot candidates from filterbank file."
    )
    parser.add_argument(
        "-r",
        "--rfi_filter",
        type=float,
        default=1,
        help="rfi-filter find_events argument [1]",
    )
    parser.add_argument(
        "-s",
        "--snr_thresh",
        type=float,
        default=10,
        help="SNR threshold find_events argument [10]",
    )
    parser.add_argument(
        "-o",
        "--offset",
        type=str,
        default="auto",
        help='offset plot_candidate_events argument ["auto"]',
    )
    parser.add_argument(
        "-n",
        "--source_name",
        type=str,
        default="bla",
        help='source_name plot_candidate_events argument ["bla"]',
    )
    parser.add_argument(
        "-P",
        "--no_plot",
    ...
    candidates = find_event.find_events(
        [dat_filepath], args.snr_thresh, False, args.rfi_filter
    )
    ...
    if candidates is not None:
        ...
        print(
            "Writing summary of %d candidate(s) to %s" % (len(candidates), log_filepath)
        )
        with open(log_filepath, "w") as fio:
            ...
            for i in range(len(candidates)):
                ...
                detectionPart = [
                    candidates.loc[rowid, "FreqStart"],
                    candidates.loc[rowid, "DriftRate"],
                    candidates.loc[rowid, "SNR"],
                ]
...
if __name__ == "__main__":
    print(
        run(
            "-P",
            [
                "/mnt/buf0/rawspec_setigen/sy_15/sy_15-ics.rawspec.0000.fil",
                "/mnt/buf0/turboseti_setigen/sy_15/sy_15-ics.rawspec.0000.dat",
            ],
            None,
        )
    )
