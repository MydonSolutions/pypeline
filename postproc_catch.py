...
def run(argstr, inputs, env):
    ...
    parser.add_argument('destination', type=str,
        help='The directory to which the RAW files are copied')
    ...
    if len(inputs) != 1:
        print('The Exceptions stage will would copy the raw files to arg.d if the rawspec run took less than %0.4f seconds.'%args.threshold)
    ...
    for procI, proc in enumerate(args.p):
        if proc.lower() == 'rawspec':
            if args.t[procI] < args.threshold:
                ...
                print('Rawspec run took %0.4fs < %0.4fs'%(args.t[procI], args.threshold))
...
if __name__ == '__main__':
    run('err-seti-node4.0/guppi_59229_47368_006379_Unknown_0001/ -t 0.011775016784667969 -p RAWSPEC', ['sadf'], None)