...
def run(argstr, inputs, env):
    parser = argparse.ArgumentParser(
        description="Copy the UVH5 toml files to a destination."
    )
    parser.add_argument(
        "destination", type=str, help="The destination directory for the copy."
    )
    args = parser.parse_args(argstr.split(" "))

    for (key, status_key) in {"proj": "PROJID"}.items():
