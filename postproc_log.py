...
PROC_INP_KEY = "LOGINP"
PROC_NAME = "log"
HASHPIPE_STATUS_KEYS = {"OBSNDROP": None}
...
def run(argstr, inputs, env):
    ...
    parser.add_argument(
        "-i", type=str, required=True, help="The instance ID (use $inst$)"
    )
    parser.add_argument(
        "-s", type=str, required=True, help="The latest output stem (use $stem$)"
    )
    parser.add_argument(
        "-b",
        type=float,
    ...
    if len(inputs) != 1:
        print(
            "The logger would log the number of detections (the output from postproc_candidate_filter)."
        )
    else:
        candidate_detections = (
            inputs[0] if inputs[0] is not None else []
        )  # pandas.DataFrame
        detection_count = (
            len(candidate_detections) if isinstance(candidate_detections, list) else -1
        )
    ...
    with open(logfilepath, "a", newline="") as csvfile:
        ...
        row = [
            ...
            "%0.3f"
            % (postproc_duration.seconds + postproc_duration.microseconds / 1e6),
            str(detection_count),
        ]
        if args.t is not None and args.p is not None:
            row.append(
                "".join([args.p[i] + "=%0.4f" % args.t[i] for i in range(len(args.t))])
            )
...
if __name__ == "__main__":
    ...
    run(
        "-H testnew -i 0 -s guppi_59366_53197_471033_Unknown_0001 -b 1622558797.2025971 -e 1622558812.1751955 -t 26.484129905700684 0.0019996166229248047 0.03121495246887207 1.5902769565582275 -p RAWSPEC Stem Logger mv rm",
        ["None"],
        None,
    )
