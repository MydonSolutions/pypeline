...
PROC_LOCAL_KEYWORD_STATUS_KEY_MAP = {"proj": "PROJID", "obsstem": "OBSSTEM"}
...
def run(argstr, inputs, env):
    ...
    if len(inputs) > 1:
        ...
        patterns = [
            os.path.join(path_head, filestem + extension) for extension in inputs[1:]
        ]

    parser = argparse.ArgumentParser(description="Move files in a detached process")
    parser.add_argument(
        "destination", type=str, help="The destination directory for the move."
    )
    parser.add_argument(
        "-i",
        type=int,
    ...
    parser.add_argument(
        "-c", type=int, required=False, help="The cpu-core to numa-bind to"
    )
    parser.add_argument(
        "-m", type=int, required=False, help="The memory-node to numa-bind to"
    )
    parser.add_argument(
        "--sync", action="store_true", required=False, help="Synchronously move."
    )
    ...
    for (key, status_key) in PROC_LOCAL_KEYWORD_STATUS_KEY_MAP.items():
        args.destination = args.destination.replace(
            "${}$".format(key), PROC_CONTEXT[status_key]
        )

    destination_normed = os.path.normpath(args.destination)
    if (
        args.destination[-1] == "/"
    ):  # normpath removes trailing '/' which split needs to identify path as a directory
        destination_normed += "/"
