...
def run(argstr, inputs, envvar):
    if len(inputs) != 1:
        print(
            "Setigen Detection Validation requires only the output from postproc_candidate_filter."
        )
        return []
    ...
    for sigI in range(len(freqs)):
        signals.append(
            {"freq": float(freqs[sigI][0:-3]) * 1000, "drate": float(drates[sigI])}
        )
    ...
    if candidate_detections is not None:
        candidate_detections = candidate_detections.sort_values(
            by=["FreqStart"], ascending=True
        )
    ...
    for i in range(max(len(candidate_detections), len(signals))):
        ...
        if i < candidate_detection_count:
            ...
            detectionPart = [
                candidate_detections.loc[rowid, "FreqStart"],
                candidate_detections.loc[rowid, "DriftRate"],
                candidate_detections.loc[rowid, "SNR"],
            ]
...
if __name__ == "__main__":
    ...
    fil_filepath = Template(
        "/mnt/buf0/rawspec_setigen/${stem}/${stem}-ics.rawspec.0000.fil"
    )
    dat_filepath = Template(
        "/mnt/buf0/turboseti_setigen/${stem}/${stem}-ics.rawspec.0000.dat"
    )
    ...
    for stem in ["sy_7", "sy_14", "sy_15"]:
        run(
            ...
            candi.run(
                "-P",
                [
                    fil_filepath.substitute(stem=stem),
                    dat_filepath.substitute(stem=stem),
                ],
                None,
            ),
            None,
